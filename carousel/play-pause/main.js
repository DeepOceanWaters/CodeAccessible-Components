...
(() => {
    ...
    function main() {
        ...
        const playPause = context.querySelector('[data-alternate]');
        ...
        for(let dot of dots) {
            dot.addEventListener(
                'click', 
                () => activateDot(dot, dots, slides, liveRegion)
            );
        }
        prevSlideButton.addEventListener(
            'click', 
            () => getNextDot(dots, -1).click()
        );
        ...
        autoPlayCarousel(
            dots, 
            () => playPause.dataset.alternate === 'play', 
            2000
        );
    }
     ...
     * @param {Event} e 
     */
    function toggleAlternate(e) {
        let el = e.currentTarget;
        let name = el.querySelector('.name');
        [name.textContent, el.dataset.alternate] =
        [el.dataset.alternate, name.textContent];
    }

    /**
     * 
     * @param {HTMLElement[]} dots 
     * @param {() => boolean} whileCondition true = autoplay, false = pause
     * @param {number} time milliseconds until move
     * @return timeout
     */
    function autoPlayCarousel(dots, whileCondition, time = 5000) {
        let timeout = setTimeout(() => {
            if (whileCondition()) {
                getNextDot(dots, 1).click();
            }
            autoPlayCarousel(dots, whileCondition, time);
        }, time);
        return timeout;
    }
})();
