...
(() => {
    // false = tabs activate on focus; true = users must activate each tab manually
    const manualActivation = false;
    ...
    function arrowNavigationKeyboardHandler(e, item, items, directions = ['horizontal', 'vertical']) {
        ...
        nextItem.focus();
        if (!manualActivation) nextItem.click();
    ...
    function getNextDot(dots, direction) {
        let currentDot = dots.find(d => d.getAttribute('aria-selected') === 'true');
